...
export const CreateCustomerLayout: React.FC = () => {
    const initValues: ICustomerCreate = {
        image: "",
        address: "",
        email: "",
        phone: "",
    ...
    const onHandleSubmit = async(values: ICustomerCreate)=>{
        setLoad(prev=>!prev)
        console.log(isLoad)
        if(!executeRecaptcha )
        return
        ...
        try{
            ...
            if(result.data === "Данний користувач вже є в системі"){
                setLoad(prev=>!prev)
                toastr.error("Замовник зареєстрований!","Помилка додавання!")
            }
            else{
                setLoad(prev=>!prev)
                toastr.success("Ви додали замовника!","Додавання замовника")
            navigator("/customers")
            }
        }
        catch(ex){
